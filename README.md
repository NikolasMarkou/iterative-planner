...
[![Tests](https://img.shields.io/badge/tests-676%20passing-brightgreen.svg)](src/scripts/bootstrap.test.mjs)
...
The test suite covers bootstrap operations, state transitions, consolidated file management, sliding-window behavior, anchor validation, and edge cases — 676 tests across 13 suites, all on `node:test` with zero external dependencies.
...
# 676 tests total: bootstrap 239, validate-plan 116, shared 71, schema 48,
...
- [ ] `node --test src/scripts/*.test.mjs` passes (676 tests, 0 failing) and `node src/scripts/check-test-count.mjs` exits 0
