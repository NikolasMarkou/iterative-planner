...
**A tool built to find accumulated residue found five defects in itself and three in the repository that hosts it, before it shipped.** The release adds a hygiene sweeper. What the sweeper actually did first was fail its own standard — twice — and then turn up a shell idiom in the Makefile under which six validation gates had been printing `ERROR` and exiting 0. That last one is the most consequential change here, so it is listed first, above the feature that found it.
...
- **`plans/ANCHORS.md` is not tracked by git at all, and this file's checklist has been corrected to say so.** That file is the durable tier of anchor resolution — the only source a `# DECISION` anchor can resolve against after its plan directory is deleted, and this repository has deleted 27 of 28. `.gitignore` carries the negation that makes it addable; nothing ever performed the add, so `git ls-files plans/` prints nothing and the durable tier does not exist in any clone. That is the mechanical cause of the seventy-five orphans above. It is **not repaired here** — it is a real change to what the repository ships and belongs in a plan of its own — but the CLAUDE.md bullet that asserted the file "is committed and stays committed" now reads as the target it is rather than a description of something true.
- **Both build channels moved together, again.** `scar-scan.mjs` is in both lint lists and `scar-scan.test.mjs` in both test lists. The validator's transition-coverage list gained the `REFLECT→EXECUTE` pair in both channels: it listed nine pairs where the Mermaid loop correctly lists ten, and the missing one is the exact edge every BoyScout remediation travels. Four enumeration floors moved with the new files — `EXPECTED_MIN_PROSE_FILES` to 23, `EXPECTED_MIN_FILES` to 20, `EXPECTED_MIN_KEYS` to 19 — each pinned by a test asserting equality with the live count, not merely a floor beneath it.
...
- Line accounting, stated in both directions: **+2643 / −54 across 18 files**, of which **+1291** is the test corpus for the two new scripts. This release is knowingly and strongly net-positive; the deliverable *is* new mechanism. The control was a bounded change surface rather than a small diff — exactly three files added, one new module, five scanner categories, no new state, no new plan-directory template, no seventh presentation contract — and it held: three added files against a budget of three.
- Test count: 940 (was 890 — net +50, live run).
