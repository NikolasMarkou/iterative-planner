...
- **What the scanner cannot see, named so nobody rediscovers it as a surprise.** A category that *runs* but silently matches nothing because its own pattern rotted is caught by per-category fixtures in the test suite and by no floor — the disclosed cost of choosing a category-count floor over a findings-count floor, since a findings floor would be a ratchet pointing the wrong way (the correct end state is zero). Category D does not read `.gitignore`. Its debug and commented-out-code rules are anchored at line start, so a debug call chained mid-line is missed — that anchoring is what stops the rules firing on prose that merely names a debug function. `console.log` and `print()` are deliberately not flagged, because every gate here is a CLI whose entire output channel is `console.log`. The collision rule's `-iter-N` exclusion hides a topic slug that genuinely collided and happens to end that way. And an anchor with no resolvable plan-id is always inherited, so an `[anchor-badprefix]` anchor a plan wrote itself reads as backlog.
...
- Line accounting, stated in both directions: **+3032 / −63 across 24 files**, of which **+1565** is test code — 1421 lines of it the suite for the new scanner alone. (The figures this entry first carried, +2643/−54 across 18 files, were measured before the release's own commits had landed and before the pre-release review sent four more findings back through EXECUTE.) This release is knowingly and strongly net-positive; the deliverable *is* new mechanism. The control was a bounded change surface rather than a small diff — exactly three files added, one new module, five scanner categories, no new state, no new plan-directory template, no seventh presentation contract — and it held: three added source files against a budget of three. A fourth path appears in the diff, `plans/ANCHORS.md`, which is not new code — it is an existing four-line file that git had never been told to track.
- Test count: 946 (was 890 — net +56, live run).
