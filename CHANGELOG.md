...
- **`checkFindingsIndexLinks` flagged a fragment-suffixed relative link as broken.** A findings-index entry like `[Auth System](findings/auth-system.md#entry-points)` — a normal way to cite a specific section of a findings sub-file — resolved against a path that still carried the `#fragment`, so the target file was never found. The trailing fragment is now stripped before the path is resolved.
- **`checkChangelogFormat` and `checkChangelogDrefIntegrity` each carried a local `if (line.startsWith("<!--")) continue;` skip that only recognized a comment's opening line**, so a multi-line `<!-- ... -->` block's interior and closing lines were misread as changelog data and warned. Both now blank comment regions with the already-audited, already-imported `stripHtmlComments` before splitting into lines — the same span-aware primitive several other readers in the file already use — instead of growing a second, function-local multi-line-aware regex.
- **That span-based fix traded the old false positive for a new false negative, and it took two review rounds to close properly.** Blanking whole comment spans means a stray, unbracketed `<!--` embedded in a data line's own reason text can pair with a later, unrelated `-->` and silently blank every line in between — including a genuinely malformed line that would otherwise have warned. The first attempt checked only whether a span's opener sat alone on its own line, which under-triggered on the mirror case (opener alone, closer sharing a line with real content) while still swallowing a malformed line in between with zero warning. A new check, `checkChangelogCommentAnomaly`, now inspects **both** ends: a multi-line span whose opener or closer fails to fully occupy its own line reports `WARN [changelog-comment-anomaly]`, naming which end is the offender. It diagnoses only — `checkChangelogFormat`/`checkChangelogDrefIntegrity` stay span-based and unchanged — and it can false-positive on a deliberate inline-appended comment, disclosed as an accepted trade-off: no syntax-only, content-blind check can tell "deliberate inline comment" apart from "accidental stray marker" from the text alone.
- **`check-register.mjs`'s `jargonMarkers` coded-ref regex disagreed with this repo's own canonical decision-id grammar.** Its local `D-\d{2,3}` / `[A-Z]-\d{3}` literals capped at 3 digits, so a 4+-digit id like `D-1000` was never counted as a coded-ref marker. It now imports and composes `DECISION_ID_NUM_PATTERN` from `shared.mjs` — the one already-imported, single-declared source of this grammar — instead of a second hand-maintained copy. Disclosed side effect, confirmed harmless: the shared pattern's 3-digit minimum is one digit stricter than the old alternative's 2-digit minimum, so a never-legal 2-digit form like `D-99` stops being counted too; density can only fall from this, never rise, which this gate's own asymmetric ratchet already treats as safe.
- **Two stale "75 inherited items" figures.** `ip-boyscout.md`'s worked example stated a specific live-looking count that had already drifted from reality; it now uses explicitly-illustrative numbers labelled as such, chosen so they can't collide with any real count. `ip-orchestrator.md`'s REFLECT-dispatch prose carried the identical hardcoded figure as a factual claim rather than an example; it now points at `plans/SYSTEM.md` for the current count instead of hardcoding a number guaranteed to go stale again.
- **A handful of SKILL.md table/diagram nits.** The File Lifecycle Matrix's `plans/ANCHORS.md` row showed `R?` at EXPLORE with no reader anywhere in `ip-explorer.md` or the orchestrator's EXPLORE dispatch — corrected to `—`. The REFLECT → EXPLORE Transitions-table caption carried a "before pivoting" qualifier no other document states — removed. The Bootstrap-shortcuts callout now says plainly that the 4 administrative edges it names are absent from the Mermaid diagram above it, rather than leaving that gap implicit. The File Ownership Model's `plan.md` row's Readers column now names `Orchestrator` explicitly, alongside Executor/Verifier/Reviewer, matching what actually reads the file at PLAN/EXECUTE dispatch.

### Changed

- **Several places where shipped prose stated a mechanism more strongly than the shipped code enforces it, now disclosed rather than left implicit.** The Autonomy Leash section now discloses its one sanctioned exception — a user-approved "continue" after a leash hit, gated every time by the user's explicit choice at PC-EXECUTE-LEASH — without weakening the existing "No exceptions" sentence, which governs unapproved continuation only. The Transitions table and Iteration Limits section now state that the enforced iteration cap counts `max(declared, derived)`, not the declared `## Iteration:` field alone, so same-iteration completion-fix and leash-override round trips can consume cap headroom the declared field never shows. The 3-Strike Rule is now marked advisory-only in both `complexity-control.md` (substantive) and `SKILL.md` (pointer) — no script counts strikes, and its "across iterations" wording doesn't cleanly cover a same-iteration retry loop. The three prose-only loop bounds (hygiene-remediation-rounds, NEEDS_EXPLORE re-dispatch, plan-rejection) now carry an explicit "self-reported, not script-counted" disclosure at their canonical location and every pointer to it. And a new paragraph names a known gap plainly: Explorer, Verifier, Reviewer, and BoyScout are documented read-only, but none of the four has any mechanical restriction on its unscoped `Bash` grant — the read-only property is prose-enforced only for that surface, and no scoped-Bash frontmatter syntax was risked on 4 production agent files without being able to confirm it's supported. None of these six additions changes what the underlying mechanism does; each corrects what the prose claims it does.

### Notes

- Test count: 986 (was 979 — net +7: 3 new regression tests for the fixes above, plus 4 from the two adversarial-review rounds that closed the comment-anomaly false negative properly; live run).
- `VERSION`/`CHANGELOG.md` were originally out of scope for this release (the findings behind it were disclosure and bug-fix work with no shipped behavior change) until the completion-fix round shipped four real behavior changes to `validate-plan.mjs`/`check-register.mjs` against a tree still labeled 2.64.0 — this entry and the version bump exist because that boundary was revisited mid-plan rather than deferred.

## [2.64.0] - 2026-09-11

**The previous release shipped an agent path that had never run, and left the sweep it added gated by nothing but orchestrator judgment.** `ip-orchestrator.md`'s REFLECT dispatch told the orchestrator, in prose only, when to spawn `ip-boyscout`, and stated plainly that a missing sweep "never blocks CLOSE." No script read the plan directory to check whether a sweep ran or was legitimately skipped before CLOSE reached it. This release closes that gap — and three rounds of adversarial review, run before the version bump specifically because a release was riding on it, found the first cut of the fix had three of its own false-positive/false-negative defects, the fix for those introduced a fourth (a regression in the very refactor meant to close the first three), and a fifth defect in a fallback path that survived being mutated to a hardcoded constant without a single test noticing — all five closed in the same release rather than shipped and discovered later.

### Added

- **`checkHygieneSweepGate`, a new `validate-plan.mjs` check that fires only in `REFLECT` or `CLOSE`.** For the current iteration it looks for one of two records: a `findings/hygiene-iter-N.md` (or `-passM` re-sweep) with a `## Verdict` heading, or a `- HYGIENE SKIP (iter N): <reason>` line inside `state.md`'s Transition History. Finding neither reports `WARN [hygiene-gate]` during REFLECT — advisory, surfacing the gap before the orchestrator recommends CLOSE — and `ERROR [hygiene-gate]` during CLOSE, where `ip-archivist` Step 1 now names a finding at EITHER severity a genuine Step-6 blocker, the same standing `[atlas-cap]` already has (the archivist's own validator run happens before `state.md` is ever flipped to `CLOSE`, so `ERROR` alone was never reachable there — see Fixed). Unlike `[atlas-cap]`, the archivist cannot self-remediate this one: it carries `disallowedTools: Agent`, so it cannot spawn `ip-boyscout` on its own behalf, and its Step 1 caveat says so — the path back is returning control to the orchestrator, not an in-place fix.
- **The gate checks that a record exists, never what it says.** A `REMEDIATE`, `REPORT_ONLY`, or `SCAN_UNTRUSTWORTHY` Verdict satisfies it exactly as a clean one does — the presence of the `## Verdict` heading is the whole signal, content is irrelevant. This is deliberate and is `scar-scan.mjs`'s own "report, never gate" contract, carried through rather than violated by proxy: a bad sweep result still never blocks CLOSE by itself, only the total absence of any sweep record does.
- **The skip-line convention, `- HYGIENE SKIP (iter N): <reason>`, lives inside the already-existing Transition History bullet list, not a new `state.md` section.** This plan (`plan-2026-09-11T141919-e5db2894`) considered and rejected a new `state.md` section for the same reason it considered and rejected a new `--pre-close`-style CLI mode with its own reserved exit code (D-001, D-003): both are heavier and riskier than reusing machinery this repository has already proven — the existing `currentState === "CLOSE" ? "ERROR" : "WARN"` severity escalation, already load-bearing at `checkVerdictBullets`, needed no new exit code and no second `--help` block. When a skip line is written, its reason is now relayed verbatim to the user in the PC-REFLECT chat block, not left to live only inside the gitignored `plans/` tree (see Fixed).

### Fixed

- **`ip-orchestrator.md`'s "never blocks CLOSE" claim was half true and, left alone, would have contradicted the gate this release adds.** It now states the distinction plainly: sweep *content* is still purely additive and never gates CLOSE on its own; sweep *absence* — no report and no skip line for the current iteration — now does, via `[hygiene-gate]`. The identical correction was mirrored into `src/scripts/modules/state-reflect.md`, the per-state rule body the single-threaded path reads with no agent definitions installed at all, so the imperative to write a skip line at the moment of a legitimate skip decision is reachable on both paths, not only when `ip-orchestrator.md` is spawned (D-004).
- **An adversarial review, run at iteration 1 specifically because this release was about to ship, found the skip-line design's own "arrow-free by construction" claim was only true for the line's prefix, not its reason text — and reproduced two false results from it, plus a third the design's own falsification test never actually exercised.** A skip-line reason containing `->` or `→` (an entirely natural thing to write in a codebase whose own prose is full of `FROM → TO` sentences) reached through `checkStateTransitions`'s lazy capture group and produced a spurious `ERROR [transition]`; the same reach-through let a reason mentioning `EXECUTE → REFLECT` inflate `countExecuteReflect`'s derived iteration count and self-invalidate the very skip line it was attached to. Separately, and more seriously: `checkHygieneSweepGate` computed its target iteration as `max(declared, derived)` — copied from the `--pre-step` leash gate, where over-counting is the *safe* direction — but here over-counting is the unsafe one. A same-iteration completion-fix round trip (`REFLECT → EXECUTE → REFLECT`, routine practice in this protocol and never a new iteration) advances the derived count past the real one, so a genuinely correct, current hygiene report could stop satisfying the gate and produce a false CLOSE-blocking `ERROR` the archivist has no way to clear. The plan's own Pre-Mortem had named this exact risk and required a falsifying test before it could be considered closed; the test written for it only varied the *declared* iteration field and never built a real round-trip history, so it passed without testing the thing it was written to test.
- **A second review round found the first fix was incomplete rather than wrong: two more Transition-History readers, `checkExplorationConfidence` and `countRePlans`, were equally vulnerable to the same arrow-reach-through, and the iteration fix traded its false positive for a false negative** — an unparseable or missing declared `## Iteration:` field made the gate return silently forever instead of ever firing. All four readers (`checkStateTransitions`, `countExecuteReflect`, `checkExplorationConfidence`, `countRePlans`) are now routed through one shared helper, `stripHygieneSkipLines()`, that excludes a `HYGIENE SKIP` line wholesale before any of the four ever runs its arrow/pair regex — not a hand-maintained list of per-function patches. The declared-iteration field now falls back to a derived count (`countRePlans`/`deriveConvergenceIteration`, immune to the completion-fix-loop inflation the original `max()` approach caused) whenever it is missing or unparseable, instead of disabling the gate.
- **A third review round found the second round's own fix had introduced a new regression, plus two further gaps.** The `stripHygieneSkipLines()` refactor reordered dash-variant normalization (en-dash –, em-dash —, hyphen U+2010) to run AFTER the line filter instead of before it in two of the four readers — so a skip line bulleted with a non-ASCII dash (visually identical to a hyphen) was no longer recognized as a skip line, and its arrow-containing reason leaked back through, reopening the exact class of bug the second round closed. Dash normalization is now centralized inside `stripHygieneSkipLines()`'s own filter predicate (and mirrored in `checkHygieneSweepGate`'s separate existence-check regex, which has its own dash-sensitivity), so no caller can reorder it again by construction. Separately: a declared `## Iteration: 0` — `bootstrap.mjs`'s own written default for a brand-new plan — parsed successfully as a number, so it bypassed the fallback added in the second round and silently disabled the gate; `0` (and any negative value) now falls back to the derived count the same way an unparseable field does. And the derived fallback's actual VALUE was pinned by no test — mutating `deriveConvergenceIteration` to a hardcoded `1` passed the full suite — so a new test now asserts the fallback derives the SPECIFIC value a hand-built two-record history produces, not merely that some fallback occurs.
- All three rounds are covered by regression tests built the same way: a concrete fixture reproducing the reviewer's exact reported shape, run against both the fixed tree and, where practical, the pre-fix tree, so a passing test is evidence the specific defect is closed rather than evidence the suite merely grew.
- **The archivist's `[hygiene-gate]` blocker check watched for a severity it could never actually see.** `# Current State:` doesn't read `CLOSE` until `bootstrap.mjs close`'s own Step 6, which runs *after* the archivist's Step 1 validator pass — so Step 1 can only ever observe `WARN`, never the `ERROR` its blocker bullet was keyed on. As written, the blocking half of the gate was unreachable. The archivist now treats a `[hygiene-gate]` finding at either severity as the Step-6 blocker: by the time Step 1 runs, the user has already confirmed CLOSE, so any finding at that point means the record was never made, regardless of which label the check happened to emit.
...
- Test count: 979 (was 949 — net +30, live run across all three review rounds; `TEST_COUNT` synced mechanically to the live `node --test` pass count).
- This entry itself was rewritten across all three completion-fix rounds to keep describing the release honestly as the work progressed, rather than being finalized after round one and left to drift stale against rounds two and three — the same discipline the 2.63.0 entry below models for its own three-round remediation.

## [2.63.0] - 2026-09-09

**A tool built to find accumulated residue found five defects in itself before it shipped, and then an adversarial review found four release-blocking ones that the tool could not see.** The release adds a hygiene sweeper. What the sweeper did first was fail its own standard — twice — and then turn up a shell idiom in the Makefile under which six validation gates had been printing `ERROR` and exiting 0. What the pre-release review then did was demonstrate two live paths on which the sweeper still hands back a clean report over a dirty tree, catch a build regression that this release had introduced into every fresh clone, and point out that the release's single highest-consequence one-word edit was pinned by nothing. That review returned **twenty concerns — four CRITICAL, nine WARNING, seven NOTE**. All four CRITICALs are closed. Six of the nine WARNINGs are closed; the rest are carried forward deliberately and are named under Notes below, rather than left to look like an exhausted list. A second review, over the repairs themselves, found two more defects — one of them a live false all-clear created *by* the fix for the first round's headline finding — and both are closed in a third round. So this took three remediation rounds, not one, and none of the nine defects was found by a gate. That is the honest summary of this release, and it is a better one than a clean first pass would have been. The Makefile finding is still the most consequential change here, so it is listed first, above the feature that found it.

### Fixed

- **Six gates in `make validate` could not fail the build, and one of them was the gate this file's own checklist cited as proof that gates bite.** The recipe shape is `for x in ...; do <test> || (echo "ERROR" && exit 1); done`. The `exit 1` runs inside a subshell, so it ends the subshell and nothing else; a shell `for` loop's exit status is the status of its **last** iteration. Whichever entry happened to be listed last was the only one enforced, and every earlier entry could fail silently while the target went on to print `Validation passed!`. The six: the SKILL.md cross-reference check, the bootstrap file-list check, the bootstrap directory check, the agent-frontmatter check, the Mermaid-edge check, and the transition-coverage check. Two of those are named in this repository's own validation checklist, which asserted that removing an edge from the state diagram "must turn the build red." It did not — the edge was removed as a probe and the build passed. All eight call sites across the six loops now use a brace group, and each of the six was probed individually by breaking a **non-last** iteration: all six now exit non-zero. `build.ps1` never had this defect; it accumulates into an error count and exits 1 when the count is non-zero, which is the other half of the "both channels fail on the same conditions" invariant, and the mirror image of the historical `Test-Path` skip bug where the roles were reversed.
- **The scanner's first live run cried wolf, in exactly the way it exists to prevent.** It reported three anchors in `validate-plan.test.mjs` as newly introduced by this work. They were not: the identical anchors sit verbatim at the release's base commit, moved down a few lines because tests were added above them. The cause was that only half of the stated partition rule had been implemented — an item was being called introduced because its *file* appeared in this plan's changelog, without the second clause requiring an anchor's plan-id to resolve and equal the active plan. A rule that turns every pre-existing finding in an edited file into a regression is this tool's own failure mode, inverted, and it gets worse the more a plan touches. Category A now partitions by anchor identity alone; file membership is proximity, not attribution. A genuinely new orphan is still caught, because a new orphan carries the active plan-id.
- **It also flagged the reviewer's own artifacts as residue.** The orphan-collision rule matched any filename ending `-2.md`, so `findings/review-iter-2.md` — a protocol-assigned name — read as a stray collision file, and the hygiene report this release introduces would have been flagged the same way from iteration 2 onward. Protocol-assigned `-iter-N` and `-passN` segments are now excluded. A rule that fires on correct names is a rule that gets ignored. Two smaller honesty defects went with it: the `[scan-floor]` branch was unreachable because the count it tested was a constant, and `--self-check` printed `5/5 categories ran` while two categories were reporting themselves unavailable — a false all-clear in miniature, in a tool whose header argues at length that a false all-clear is the worst thing it can produce.
- **This release broke `make test` in every fresh clone, and the break was found by verification rather than by a gate.** Four tests in the new scanner suite read the developer's live `plans/` directory, which is gitignored — so `git archive HEAD | tar -x && make test` gave 936 passing with 4 skipped against a `TEST_COUNT` of 940, and failed. The same procedure at the release's base commit gives 890 of 890 with no skips. The four tests now build their own plan-directory fixture in a temporary tree, the way the rest of the suite already did, so the suite is environment-independent and `TEST_COUNT` means one number everywhere. The tempting repair — counting skips as passes — was refused: it would let any future test disable itself in silence.
- **The scanner's central claim, that a format drift in its upstream cannot produce a false all-clear, was false.** Proof of the upstream having run was taken from its summary line alone. Drift the *issue* lines, which are the ones the parser actually consumes, and leave the summary intact, and the anchor category reports zero findings, all five categories `ran`, exit 0 — over a repository with seventy-five orphaned anchors in it. The summary line already carries the upstream's own error and warning counts, so those counts are now corroborated against the number of issue lines the parser recognized: an upstream that reports problems while the parser sees none is `[scan-unavailable]`, not a clean sweep. Two sibling honesty repairs went with it. Category E was returning `ran` on two branches whose own note says nothing was measured — the exact defect the collision-rule fix above claims to have repaired, still live one category over — and now returns `degraded` on both. Category D reported a clean sweep over zero discovered files; zero files is now a degraded discovery. Together these were the difference between an empty fixture repository containing a real dead import and a real `TODO` reporting `no category degraded`, and reporting the truth.
- **Nothing mechanical guarded the one edit this work called its most consequential.** The new agent is safe because it is read-only and because the orchestrator can spawn it, and both facts live in one line of frontmatter each. No gate read `disallowedTools` at all, and nothing pinned the orchestrator's `Agent(...)` grant, so deleting `disallowedTools: Edit, Agent` from the agent — or dropping it from the grant — passed `make validate` and `make test`. Two tests now pin both, and both derive their subject list from `src/agents/` on disk rather than naming the eight agents, so a ninth agent is checked rather than ignored: every agent file except the orchestrator's must appear in the grant and every name in the grant must be an agent file, and every agent must state each of `Edit` and `Agent` exactly once — granted in `tools:` or refused in `disallowedTools:`. A read-only agent can satisfy the second rule only by carrying the refusal, so losing the line turns the suite red. Both were confirmed to fail before they were committed.
- **The fix that recorded a budget breach silenced the finding that detected it.** The scanner declared its own, stricter copy of a grammar `validate-plan.mjs` already owned for the same `plan.md` line: it required the closing `**` immediately after `max`, where the validator tolerates trailing text. Rewording the budget bullet to satisfy the validator therefore moved four characters past the scanner's regex, the parsed cap went null, and the Complexity Budget category reported a clean `ran` — `All categories ran. No category degraded.` — over a plan whose own line read `OVER BUDGET`. A false all-clear, in the category that round was fixing, produced by that round, on this tool's own plan. The grammar now has exactly one declaration, in `shared.mjs`, imported by both readers and pinned by a source grep asserting no second copy exists — the same discipline that already protects the changelog field regexes. And "cap present but unparseable" is now a third degraded arm for that category, because an unparseable cap is an unmeasured cap; a budget stated in prose alone still reports `ran`, since it claims no cap to miss.
- **A test whose result depended on where `$TMPDIR` pointed.** The previous round moved four tests off the developer's live `plans/` directory and onto fixtures of their own — and those fixtures inherited their git condition from `$TMPDIR`, which sits inside a git work tree on plenty of real machines. Git's discovery walked out of the fixture, found that unrelated repository, and a root staged as "no git" silently became "git present": the scanner suite gave 50 of 50 in one environment and 47 pass / 1 fail / 2 skipped in the other. Each fixture now states the condition it needs — `GIT_CEILING_DIRECTORIES` for the git-less ones, a real repository with a tagged commit for the one that asserts every category ran — and the suite's own git probe runs under the same ceiling as the process under test, so the two cannot disagree. Two `t.skip` branches went with it. The suite is identical, 50 of 50, with and without the override.
- **Two numbers this work was planned against were wrong, and both were corrected rather than carried.** The claimed four stale `uncommitted` commit fields in the most recently closed plan are really **two**: the baseline used `grep -c uncommitted`, which counted the word wherever it appeared, including in two append-only correction entries whose *reason* text mentions it. Splitting the pipe-delimited fields properly finds two. And the orphaned-anchor backlog is **75**, not 64 — the 64 `[anchor-unknown-plan]` errors the baseline counted, plus 11 `[anchor-badprefix]` warnings it never did.

### Added

- **An eighth agent, `ip-boyscout`, which cleans up after a plan without being able to touch anything.** It runs inside REFLECT after an adversarial review returns `READY_TO_CLOSE` — or, when no reviewer ran that iteration, at the moment reached a different way: every verification criterion passing with no regressions. It writes one artifact, `findings/hygiene-iter-N[-passM].md`, splitting every finding into what this plan introduced and what was already there. It carries `disallowedTools: Edit, Agent` and cannot change a line of anything. **The indirection is the point, so state the reason plainly:** `verification.md` and the reviewer's diff review both describe the tree *as it stood when they captured it*, and the user approves that description at PC-REFLECT. An agent that edits after that moment silently invalidates evidence someone has already approved, and nothing in the protocol re-triggers verification. So cleanups are minted as numbered completion-fix sub-steps — `iter-N/step-M.K`, attributed to the step whose changelog line named the file — and applied by the ordinary executor through the existing REFLECT-to-EXECUTE edge, after which REFLECT runs again over whatever changed. An item that cannot be attributed to a single step is reported and never minted; inventing a step number to satisfy the changelog grammar is how `iter-1/completion-fix` got written twice before. Remediation is bounded at two rounds per iteration; a third sweep reports and mints nothing.
- **`scar-scan.mjs`, the mechanical half of `code-hygiene.md`.** Five categories: orphaned decision anchors, stale `uncommitted` changelog commit fields, orphan plan-directory artifacts, the Forbidden Leftovers code sweep, and Complexity Budget claims reconciled against the real diff. The anchor category **delegates** — it shells out to `validate-plan.mjs` and parses its report rather than re-declaring the anchor grammar, the same prohibition that already protects the changelog field regexes. That coupling is made safe by refusing to fail open: the scanner demands positive proof the upstream actually ran, and emits `[scan-unavailable]` on stderr with **nothing on stdout** otherwise. Findings never fail a build. The only two things that exit non-zero are the two ways the scan itself can become untrustworthy, which is why it is deliberately not in `make validate`.
- **A third artifact schema in `findings/`.** `validate-plan.mjs` discriminated hygiene reports against the *explorer* schema, which would have required a decorative "Code Patterns" heading while leaving the routing-critical `## Verdict` ungated. A `HYGIENE_FILE_RE` branch now lints them against `Inherited` / `Introduced` / `Verdict`. The discriminator switches the required list; it never exempts a file.

### Changed

- **Seventy-six inherited items are reported and deliberately left alone.** Seventy-five orphaned decision anchors across ten dead plan directories, plus one dead import. Auto-remediating inherited backlog inside an unrelated plan is precisely the scope creep the protocol forbids, and the partition exists so that backlog can be reported forever without reading as a regression. The count is labelled, each dead plan-id gets a suggested standalone `bootstrap.mjs retire` command, and nothing is minted.
- **`plans/ANCHORS.md` is now tracked by git, for the first time.** That file is the durable tier of anchor resolution — the only source a `# DECISION` anchor can resolve against after its plan directory is deleted, and this repository has deleted 27 of 28. `.gitignore` has carried the negation that makes it addable for some time; nothing ever performed the add, so `git ls-files plans/` printed nothing and the durable tier did not exist in any clone. That is the mechanical cause of the seventy-five orphans above. The first instinct here was to report it and leave it, on the rule that inherited backlog is never repaired inside an unrelated plan — and the review supplied the fact that changes the answer: this release does not merely decline to shrink that backlog, it adds five new anchors to it. Committing a four-line file the repository's own ignore rules were already written to permit is a one-command repair with no design surface. `git ls-files plans/` now prints exactly `plans/ANCHORS.md` and nothing else. This stops the backlog growing; it does not shrink it. The seventy-five stay reported, and the manifest still postdates almost every one of them.
- **Both build channels moved together, again.** `scar-scan.mjs` is in both lint lists and `scar-scan.test.mjs` in both test lists. The validator's transition-coverage list gained the `REFLECT→EXECUTE` pair in both channels: it listed nine pairs where the Mermaid loop correctly lists ten, and the missing one is the exact edge every BoyScout remediation travels. Three enumeration floors moved with the new files — `EXPECTED_MIN_PROSE_FILES` to 23, `EXPECTED_MIN_FILES` to 20, `EXPECTED_MIN_KEYS` to 19 — each pinned by a test asserting equality with the live count, not merely a floor beneath it.

### Notes

- **Cost accounting** (per the CLAUDE.md rule on full-corpus walks): **this release adds no walk over `plans/`.** Categories B, C and E read a single plan directory — the active one by default — and are O(1) in plan-directory count. Category A delegates to `validate-plan.mjs`, whose own O(referenced-plans) cost is already documented and unchanged. Category D is O(source files in scan scope), a tree walk under a fixed skip set with no plan-directory enumeration in it.
- **The blind spot that cost buys.** Category B's active-directory-only scope means a default run is blind to stale commit fields in every *closed* plan — which is exactly where they accumulate, since a field can only go stale after its plan stops being edited. The scanner can see them, but only when someone points `--plan-dir` at a closed directory, and nothing in the protocol does that automatically. Widening the default would have restored the plan's original acceptance number at the price of a full-corpus walk over `plans/`; that trade was declined.
- **What the scanner cannot see, named so nobody rediscovers it as a surprise.** This list is the summary of the one in `scar-scan.mjs`'s own header, and it is meant to say the same things. A category that *runs* but silently matches nothing because its own pattern rotted is caught by per-category fixtures in the test suite and by no floor — the disclosed cost of choosing a category-count floor over a findings-count floor, since a findings floor would be a ratchet pointing the wrong way (the correct end state is zero). Category D does not read `.gitignore`. **All three** of its line rules — marker comments, debug statements and commented-out code — are anchored at line start, so a debug call chained mid-line is missed and so is a marker in a *trailing* comment, which is the commonest form of it in real code; dropping the anchor from the marker rule was measured on this repository and produced three findings, every one a marker string quoted inside the scanner's own test fixtures. `console.log` and `print()` are deliberately not flagged, because every gate here is a CLI whose entire output channel is `console.log`. The collision rule's `-iter-N` exclusion hides a topic slug that genuinely collided and happens to end that way. An anchor with no resolvable plan-id is always inherited, so an anchor a plan wrote itself but mis-prefixed — or wrote with no plan-id at all — reads as backlog. And the upstream-corroboration check added in this release catches **total** issue-line drift, not **partial**: if other checks' lines still parse while the anchor checks' lines do not, the parsed count is non-zero and the anchor category still reports a clean zero. The stricter rule (parsed count equals declared count) would close that and was deliberately not taken.
- **The installed skill bundle is not updated by a release.** Installing is a separate action the user runs (`make sync-skill`); nothing in this work wrote anything under `$HOME`.
- **The agent path in this release has never run.** Because nothing is installed until `make sync-skill`, the new agent has never been spawned, the orchestrator's hygiene-sweep step has never executed, no cleanup has ever been minted as a numbered sub-step, and no `findings/hygiene-iter-N.md` has ever been written by anything but a test fixture. Roughly two hundred and fifty lines of new agent and orchestrator instruction text ship with no runtime feedback at all. The scanner underneath it is exercised hard — the suite plus the review's own probes — but the sweep as a *procedure* is not validated by this release and should not be read as such. The first REFLECT after a sync is its smoke test.
- **What is knowingly carried forward, named rather than dropped.** Four items from the first review and five from the second are open on purpose. An anchor this plan itself wrote but mis-prefixed is classified inherited and can sit under the ten-item display cap, which is the likeliest new-anchor mistake an agent makes and deserves its own labelled line above the cap. The two-hygiene-rounds-per-iteration bound is stated three times in prose and counted by nothing; it should be derived from the count of `findings/hygiene-iter-N*.md` files rather than remembered across REFLECT re-entries. The `# DECISION` anchor this release put in the `Makefile` is unscannable, because `Makefile` is not in the validator's anchor source extensions — the consequence is twelve permanent `[anchor-refs-stale]` warnings on this plan's own validator output, which is precisely the warns-on-correct-input shape this repository deleted an entire check for. `ip-boyscout.md` tells the agent every reported item carries `severity`, and two item shapes do not; nothing schema-tests the `--json` report the agent's instructions depend on. And `bootstrap.test.mjs` carries a pre-existing intermittent failure under full parallel load — subtest `(2b)`, `TypeError: The "path" argument must be of type string. Received null` — whose root cause is that the suite's `run()` helper never asserts `execFileSync`'s exit code, so a `bootstrap new` that times out is swallowed and the test then dereferences a null plan directory. It is unrelated to this release, it did not reproduce in four consecutive fresh-export runs, and it is written here rather than only in a gitignored plan directory so the next person to meet it does not have to rediscover it.
- Line accounting, stated in both directions: **+3184 / −72 across 26 files**, of which **+1670** is test code — 1493 lines of it the suite for the new scanner alone. (This entry has now been remeasured twice: it first carried +2643/−54 across 18 files, taken before the release's own commits had landed, and then +3032/−63 across 24 files, taken before a second review sent two further findings back through EXECUTE.) This release is knowingly and strongly net-positive; the deliverable *is* new mechanism. The control was a bounded change surface rather than a small diff — one new module, five scanner categories, no new state, no new plan-directory template, no seventh presentation contract. The file cap did **not** hold, and the tool caught it: the plan budgeted three added files and the diff adds four. The fourth is `plans/ANCHORS.md`, which is not new code — it is an existing four-line file that git had never been told to track — but the budget's unit is files added to the repository, so the count is four, the plan records `4/3 max — OVER BUDGET`, and the scanner still reports the breach on every run. Redefining the unit to exclude it would have restored a clean three and would have been exactly the definitional retreat the rest of this release argues against.
- Test count: 949 (was 890 — net +59, live run).

## [2.62.0] - 2026-09-01

**The anchor scanner was fixed three times, and each fix bought a new way to lose an anchor. This release stops fixing it and takes something away instead.** After the previous release, an adversarial review re-checked the work and found that three of those fixes had introduced defects of their own. Two were in the same component — the code that finds `# DECISION` comments in source files — and that component had now broken on every attempt to improve it. So it was not patched a fourth time. The scan was narrowed to the languages it was ever valid for, and the cleverest part of it was deleted.

### Fixed

- **The anchor scanner no longer runs on languages that have no `/* */` comments at all.** Of the 33 file extensions the anchor tools scan, 13 — `.py .rb .sh .bash .zsh .yml .yaml .toml .r .pl .pm .tf .sql` — have no C-style block comments, yet the block-comment scan ran on all of them, because it was gated by complement (everything except the markdown family). A four-line shell script with two ordinary globs around a `# DECISION` line — `rm -rf build/*` above it, `cp src/*/lib dest/` below — therefore opened a phantom comment span: the validator reported that anchor **twice** while `bootstrap.mjs retire` stamped it **once**. The gate is now an explicit allowlist, `BLOCK_COMMENT_EXTS`, defined once and consumed by both tools, and a test asserts that every scanned extension falls into exactly one of block-scanned / HTML-style / line-marker-only — so a future extension cannot be added without classifying it.
- **Anchors inside JSX comments are visible again.** `{/* DECISION … */}` on a line that also carries `</div>` or `/>` returned *no* span at all, because the regex-literal lexer added by the previous release read the `/` of `/>` as the start of a regular expression and swallowed the comment opener. That is a silent loss: neither the validator nor `retire` sees the anchor, and nothing anywhere reports that anything is missing. The lexer was **deleted, not corrected** — three strikes in one component means the approach is the defect. Seven anchors came back in the test corpus; the JSX probe went from 0 reports to 1.
- **The validator no longer opens with four errors of its own making.** Two JSX span tests added by this release quoted the live plan-id with invented decision numbers, so the anchor audit read them as orphaned anchors — pointing at decisions that do not exist — and every `validate-plan.mjs` run in this repository began with four `[anchor-orphan]` ERRORs. The neighbouring pins in the same file already used the `p/D-NNN` fixture convention; these now match it. Left alone it would have blocked a future plan's REFLECT-to-CLOSE gate on errors that were never about that plan.
- **`check-register.mjs --regenerate` will no longer raise a ceiling nobody named.** The register gate exists to stop jargon density from creeping upward, and its baseline of per-file ceilings is regenerated by a flag — which, until now, silently rewrote a ceiling *upward* as readily as downward. So the way past the ratchet was to run the tool the ratchet's own failure message told you to run. `--regenerate` now lowers or holds any ceiling freely and refuses to raise one unless the file is named with `--raise <path>`, exiting non-zero and listing the files whose ceiling would have risen. Deletion immunity is preserved on both sides: the gate never fails on a deletion, and a density rise whose raw marker count FELL needs no flag either — deleting jargon is not raising anything. What the guard does **not** do, said here because the gate's own PASS text used to claim otherwise: it guards one code path to the baseline, not the file. Hand-editing `register-baseline.json` to the live number reaches green through this gate and through the whole suite. What catches a hand-edit is the pin test's demand that every committed ceiling *equal* its file's live measurement — so a hand-edit survives exactly when it writes what `--regenerate` would have written.
- **The `[convergence]` warning stopped firing on plans that had not re-planned.** The check derived "which iteration is this" from a counter that increments on every `EXECUTE → REFLECT` transition, so a completion-fix loop — REFLECT sends work back to EXECUTE within the same iteration — pushed it to 2 and tripped a warning aimed at plans that have genuinely gone around again. The advisory check now counts only the arrivals at PLAN that the protocol's own transitions table calls a new iteration: those reached by *leaving* REFLECT, i.e. `REFLECT → PIVOT → PLAN` or `REFLECT → EXPLORE → PLAN`. A plain arrival count was tried first and was worse — it read `PLAN → PLAN`, which is what happens every time a user asks for a plan revision, as iteration 2. The iteration **hard cap** deliberately keeps the old raw counting and was not touched: it is a safety limit, and it is allowed to over-count but never to under-count.

### Changed

- **The scanner over-reports now, and that is the chosen behavior.** Deleting the regex lexer re-opens the shape it was written for: a regular expression like `/[*/]/` supplies both a `/*` and a `*/`, so the scanner sees a comment where there is code. This is pinned by tests whose assertion messages carry the decision id, so nobody "fixes" it back. The reasoning is one asymmetry: **an over-report appears in the validator's output where a maintainer sees it; a silent loss is invisible to the validator, to `retire`, and to the audit trail the whole feature exists to produce.** Backtick handling was removed for the same reason — a lone backtick inside a regex literal used to start a template-literal skip that masked real comment spans, which is a loss shape, not a visible one. **That removal has a cost this entry originally left unstated, so state it plainly: the two-reports/one-stamp desync above is closed for the hash family, not everywhere.** A template literal is no longer a string to this scanner, so a shell glob inside one supplies a lone `/*` that a real comment further down closes — `` const clean = `rm -rf build/*`; `` above a `// DECISION …` line puts the anchor inside a phantom span, and the validator reports it twice while `retire` stamps it once. The same shape as the shell script, in a `.mjs` file. It is an over-report, never a loss (the anchor is found by both tools, and `retire` may over-stamp but never under-stamp), and it is now pinned with its report and stamp counts asserted rather than assumed to be the benign balanced case.
- **The cost of that trade, measured rather than asserted.** In this repository the change produces **15 newly extended or phantom spans**, including the two live cases the previous release named. No live anchor is lost or duplicated. The validator's own warning floor on this project rose from **16 to 27**, the extra 11 being `[anchor-badprefix]` warnings on template literals inside test fixtures — a real cost, because a louder floor can hide a future genuine warning. (The 14-to-25 figure this entry first carried was arithmetic from a briefing, not a re-run; two independent agents reproduced 16 and 27.)
- **Anchors written inside `/* */` comments in `.tf` and `.sql` files are no longer found.** HCL and SQL do have block comments, so gating them off is a real loss, traded away with the rest of the family. Anchors in those files' native `#` and `--` line style are unaffected, and that is the style the anchoring guidance has always asked for. Disclosed and tested rather than discovered later.
- **Two documentation claims that were false have been replaced with claims that are checked.** `decision-anchoring.md` described the block scan as "gated by complement", which stopped being true in this release. The comment in `shared.mjs` listing the scanner's known holes stated the wrong *direction* for both of its bullets — it claimed a phantom opener hides anchors when it duplicates them — and was rewritten from live reproductions. A test in `emit-template.test.mjs` described the `[header-copy]` rule as scanning up to a boundary marker that the rule does not use; it now states what the check does, and a new complement test proves the silence outside the scanned region is not vacuous.
- **The one authorized exception to "decisions.md is append-only" is now stated identically in all three places an agent might read it** — the file-format reference, the protocol, and the archivist's own instructions: `ip-archivist` may backfill a past entry's `Anchor-Refs` line at CLOSE, and nothing else may edit history.

### Notes

- **Cost accounting** (per the CLAUDE.md rule on full-corpus walks): **no change in this release adds a walk over `plans/`.** Verified by diffing every `src/scripts/` change since the previous release for directory enumeration — `readdir`, `opendir` or glob expansion appear in no added line.
- **The installed skill bundle is not updated by a release.** Installing is a separate action the user runs (`make sync-skill`); nothing in this work wrote anything under `$HOME`.
- Line accounting, stated in both directions rather than averaged: the scanner shrank (`shared.mjs` **−11 net**, with the lexer's ~55 lines gone), while the register guard and the convergence derivation added **+193 net** across the other three scripts. Test lines rose by **+589 net** — the acceptance corpus for a trade like this one is the deliverable.
- Test count: 890 (was 852 — net +38, live run).

## [2.61.0] - 2026-09-01

**Two defects in this release were silently corrupting real state, and neither announced itself.** A comprehensive four-pass review of the runtime scripts, the validator, the gate scripts, the build harness and the protocol prose found roughly thirty real defects and five *wrong* institutional-memory records — records that described the codebase's behaviour backwards, so anyone reading them was steered away from the actual bug. Everything below is fixed, with the two state-corrupting ones first.

### Fixed

- **Every `bootstrap.mjs close` merged a phantom `D-001` into the cross-plan decision archive — and that defeated anchor-typo detection.** `stripHeader()` found the merge cut point with a raw `^## ` search. In bootstrap's own `decisions.md` template the first `^## ` line is the `## D-001 | EXPLORE → PLAN | YYYY-MM-DD` *schema example*, which sits inside an HTML comment. So every close decommented that example and appended it, plus a dangling `-->`, to `plans/DECISIONS.md`. Because `plans/DECISIONS.md` is a tier the validator resolves `# DECISION` anchors against, a fabricated `<any-closed-plan>/D-001` anchor then resolved clean — the typo check the tier exists to provide was gone for that id. The cut point is now located in the comment-stripped text while the *original* lines are returned from that point on, so real content — including agent-authored comments inside decision bodies — still merges byte-identically. This repo's own `plans/DECISIONS.md` carried two such phantom blocks; they were removed.
- **`ensureGitignore` could leave `plans/ANCHORS.md` un-committable.** `.gitignore` semantics are last-match-wins, so the `plans/` glob and the `!plans/ANCHORS.md` negation are an *ordered* pair, and the writer did not treat them as one. On a `.gitignore` that already carried a negation, or that carried the two inverted, the durable tier of anchor resolution silently stopped being tracked — and in a fresh clone every other tier lives inside the ignored plans directory, so anchor resolution would have had nothing left to resolve against. The invariant `lastIndexOf(negation) > lastIndexOf(glob)` is now established by placement in all five reachable `.gitignore` shapes.
- **The block-comment anchor scanner both double-reported and dropped anchors, and the recorded explanation of it was backwards.** `plans/SYSTEM.md` and `plans/LESSONS.md` both recorded that a phantom `/*` opener "silently swallows every `# DECISION` anchor until the next `*/`". Two independent reproductions found the opposite: a phantom opener causes **double-reporting**; the silent-loss shape is a phantom **closer** (`*/` appearing as prose *inside* a genuine block comment, which ends the comment early). `shared.mjs` gains `blockCommentSpans()`, the `/* */` twin of the existing `htmlCommentSpans()` — it skips string literals, `//` line comments and backslash escapes, yields no span for an unterminated `/*`, and recovers from a prose closer — and both `findAnchorsInFile` and `retire`'s non-markdown stamper now consume that one computation. On this repo the anchor count went 45 → 49 with zero removals and zero duplicates.
- **Three validator WARNs fired on conforming input.** (1) A reviewer file (`review-iter-N.md`) was checked against the *verifier's* required-section list, so every real reviewer output was warned; the section list now switches on the filename and is derived from `ip-reviewer.md`'s own Output Format. (2) A verification row whose Criterion was still an unfilled placeholder was warned for having no Verdict; placeholder rows are now skipped, reusing the existing `PLACEHOLDER_PATTERNS` rather than adding a second exemption list. (3) One check was removed outright rather than repaired, leaving a named tombstone at its site. Validated against an independent corpus of two prior real plan directories and three real reviewer files: three false WARNs cleared, nothing else moved.
- **The changelog op-field shape was restated wrongly in four places** — `shared.mjs`, two sites in `bootstrap.mjs` (including the served `changelog.md` template header), and a comment in `validate-plan.mjs`. Each copy spelled the op field `OP(+N,-M)`, a shape `CHANGELOG_SPEC` rejects and which cannot express `CREATE(+N)`, `DELETE(-N)` or `RENAME(old→new)`; anyone who followed a copy wrote a line the validator warns about on every file they create. All four now name the field *order* and point at `CHANGELOG_SPEC` for the shapes, which remains the one definition.

### Changed

- **Two gate evasions closed.** `check-agent-wiring` matched `node <skill-path>/scripts/x.mjs` by token adjacency, so any intervening option flag (`node --experimental-x scripts/…`) slipped past; it now uses a command grammar. Both rules that resolve a cited reference path now share one exported normalizer, `canonicalDocPath()`, so they cannot disagree about what a citation names. `check-readme-parity` matched the version and test-count strings *anywhere* in README; both are now anchored to their shields.io image badges. A whole-repo differential confirmed both evasions were injection-only — no shipped prose was affected.
- **The two build channels are now checked for lockstep.** `Makefile` and `build.ps1` had drifted: different gate sets, `Test-Path`-wrapped gate skips that turned a missing gate script into a silent pass, and a `build-combined` pointer-rewrite map that covered `references/` but not the `emit-state.mjs` / `emit-template.mjs` invocations — so a reader of the single-file channel was told to run a script that channel does not ship. Six static assertions now pin the two channels to the same validate set, lint list, test list, `build-combined` rewrite pairs, and encoding contract, plus a state-machine Mermaid-edge assertion extended from 9 pairs to 10 (`REFLECT --> EXECUTE` was a real edge in both the diagram and the Transitions table that the old list omitted).
- **BREAKING (Windows build channel only): `build.ps1` now declares `#Requires -Version 7` and will refuse to run under Windows PowerShell 5.1**, the interpreter that ships with Windows. Use `pwsh` (PowerShell 7+). The floor is what makes the encoding contract enforceable — 5.1's `Set-Content` defaults to the ANSI codepage, which silently mangles the em-dashes and box characters in the protocol prose — but it is a compatibility break and was previously recorded only in `CLAUDE.md`. Nothing else in the project requires PowerShell 7; the Unix `make` channel is unaffected.
- **The autonomy leash was unreachable in monolithic mode**, and separately the fix-attempt *unit* was ambiguous. The `--pre-step` gate imperative now lives in `scripts/modules/state-execute.md`, the one rule body both modes read, naming the invocation, the four slugs, the per-slug action and who runs it; and `ip-executor` is capped at one fix try per spawn, so two recorded attempts in `state.md` means exactly two tries. A second reachability defect surfaced during the work: free-form attempt lines were never counted at all.
- **`check-register`'s ratchet now ratchets.** Every per-file ceiling in `register-baseline.json` carried 17%–100% permanent slack (`complexity-control.md` measured 0 against a ceiling of 2.5), and the file had exactly one commit, generated "PASS by construction" — a loose ceiling, not a ratchet. Every ceiling is re-measured at HEAD and set to the live density. `EXPECTED_MIN_FILES` also gains the `pin:` test the other three floor-carrying gates already had, binding it to the live scanned doc count; the baseline key count is pinned to the same number, so an ungated new doc or a stale ceiling fails.
- **Protocol prose reconciled with the code it describes**: changelog compression stated as lossy, the consolidated window corrected to 25 plans, the File Lifecycle Matrix reconciled with the ownership table (`decisions.md` at CLOSE and `findings/*` at REFLECT are R+W, not R), and `verification.md`'s template header no longer claims an EXECUTE write.

### Notes

- **Cost accounting** (per the CLAUDE.md rule on full-corpus walks): **no change in this release adds a walk over `plans/`.** Verified by diffing every `src/scripts/` change in the release for directory enumeration — there is none. `collectKnownDecisionIdsByPlan` stays O(referenced-plans); its new `tiersByPlan` sidecar records the resolving tier at `add()` time and reads nothing extra, so the two orphan messages can name only the sources actually read.
- **What is verified by static read only: the entire `build.ps1` channel.** No PowerShell interpreter was available in the environment where this release was developed. The six lockstep assertions compare the two files' *text*, which is what makes them lockstep assertions; but `build.ps1`'s own behaviour — every gate invocation, the encoding round-trip, the `build-combined` rewrite — is unexercised here and rests on the Makefile half plus the textual pin. Treat a `build.ps1` regression as possible until someone runs it.
- **An unresolved contradiction between two of this repo's own rules is recorded, not fixed.** Shipped `.md` files cannot carry live `# DECISION` anchors — the validator scans them and a real anchor in shipped prose turns the build red — while `references/decision-anchoring.md` and the executor protocol both mandate anchoring at the point of impact. There is currently no way to satisfy both for a decision whose point of impact is prose. Noted in `CLAUDE.md`; not resolved.
- Test count: 852 (was 816 — net +36 across the release, including the completion-fix commits that followed the first 2.61.0 tag; live run).
...
- **`plans/ANCHORS.md` — a committed, append-only anchor manifest.** One line per anchored decision: `<plan-id>/D-NNN | YYYY-MM-DD | one-line rationale`. Created by `bootstrap.mjs new`, appended by `ip-archivist` at CLOSE (its new Step 2), never edited, never reordered, never trimmed. It is the only anchor-resolution source that is not inside the ignored `plans/` tree.
- **A fourth anchor-resolution tier in `validate-plan.mjs`** that reads the manifest. It parses line-oriented, anchored at line start, requiring the pipe delimiter immediately after `<plan-id>/D-NNN`, and reusing the shared plan-id patterns so both the modern and the legacy `plan_YYYY-MM-DD_XXXXXXXX` id shapes resolve (2 of this repo's 8 anchored plan-ids are legacy, and they account for 20 of the 39 errors). Non-matching lines are ignored silently; the tier never throws. A missing or unparseable manifest degrades to exactly the pre-2.58.0 behavior — the anchor stays unresolved and ERRORs — never to a false PASS.
  - **Cost note (required for anything that touches `plans/`-corpus reads): tier 4 is O(1) file reads and O(manifest lines) parse per full validation, independent of the number of plan directories, and it never runs on the `--pre-step` path** (that gate stays suite-free and sub-50ms). It does not restore, and must not be used to justify restoring, the O(all-plan-dirs) walk removed in 2.53.0.
- **A 21-line one-time backfill** of this repo's own manifest, reconstructed from the 21 distinct `<plan-id>/D-NNN` pairs behind the 39 errors, across 8 plan-ids. Each line asserts only that the id existed and what its anchor comment says — never a reconstructed `decisions.md` entry. The block is preceded by a comment saying so.

### Changed

- **Bootstrap's gitignore pattern is now `plans/*` + `!plans/ANCHORS.md`, replacing `plans/`.** Git does not descend into an excluded *directory*, so under the old `plans/` line the negation could never fire. The two forms hide the identical else-set — every entry directly inside `plans/`, each plan directory, and dotfiles such as `.current_plan` — verified with `git check-ignore -v` over a populated tree (37 ignored paths in this repo before and after; the only newly visible path is the manifest). `ensureGitignore` migrates an existing line in place, matching **exactly** (after trimming) one of two spellings, `plans/` or `/plans/`, and nothing wider: `myplans/`, `plans/tmp`, `plans/tmp/`, `replans/` and a commented-out `# plans/` all survive byte-identical. The leading-slash spelling is accepted because a hand-written `/plans/` shadows the negation the same way, which left the manifest permanently ignored with no error.
- **The changelog `step` field grammar accepts a bounded sub-step: `iter-N/step-M.K`.** A completion fix is numbered as a sub-step of the plan step it repairs — a fix to step 9 is `iter-1/step-9.1`, a second fix to step 9 is `iter-1/step-9.2` — and the iteration does not go up. `iter-N/completion-fix` is still rejected, deliberately: it has produced `WARN [changelog-malformed]` batches in two plans in this repo. The rule is written into SKILL.md's transition table, the REFLECT module, the orchestrator's REFLECT dispatch, `file-formats.md`, and both halves of the lockstep template pair.
- **Four shipped sources said the `step` field "stays bare `iter-N/step-M`"**, which contradicted the sub-step rule above. They now say what they always meant: the field carries no plan-id prefix, and it always names a numbered step. The reason is unchanged — that field is sourced from `state.md` and nothing in this codebase reads a commit message.
- **The orchestrator's CLOSE verify list now includes the manifest append.** SKILL.md's CLOSE row already named it; the orchestrator's own confirm-list did not, so a skipped archivist Step 2 would have been undetectable and the manifest would have silently stopped growing.
- `plans/ANCHORS.md` is documented across the File Ownership tables (now 18 keys, both sides), the File Lifecycle Matrix, `decision-anchoring.md`'s resolver order, and `file-formats.md`. The old "if the plan directory is deleted (rare)" wording is corrected: deletion is not rare, and the `summary.md` fallback lives inside the very tree it was offered as a fallback for.

### Fixed

- **A nested commentary bullet whose label merely contained a required label hijacked a Verdict field.** A Verdict with five correct, filled, ordered fields plus a sub-bullet such as `- Recommendation-related follow-up: none` produced a CLOSE-blocking `bullets present but not in required order` and/or `still unfilled (PENDING)`. Verdict fields are now identified by **label tightness**: a bullet is a field when its normalized label *is* one of the five required labels (or a separator-joined compound of them, counted once). Presence, order and PENDING all read that one derived list, and PENDING is reported per required field rather than per bullet.

### Compatibility — two behavior changes a previously-passing Verdict can hit

1. **A Verdict with only four real fields, whose fifth required label appears solely in a nested decoy bullet, was silently accepted and is now reported as `missing required bullet(s)`.** That Verdict was always incomplete; the old scan was satisfied by the decoy. This is a false negative being closed, so some previously-passing plans will newly fail — correctly. It is the single `CLEAN → ERROR` transition in the differential below, and it is pinned by its own named test.
2. **Verdict field detection no longer considers indentation at all.** An earlier attempt in this release made the minimum indent among label-matching bullets load-bearing; an adversarial review found six shapes that were clean before and hard-ERRORed after (a ragged fifth field, one tab-indented field among four space-indented ones, five fields nested under a lead-in bullet carrying the first required label). Indentation is gone as a discriminator, and the practical consequence is the trade this release accepts: a bullet whose label is a required label **plus extra words** — `Recommendation-related follow-up`, `Criteria passed summary`, `Regressions found` — is commentary, not a field, wherever it sits. A Verdict labelled that way now reports exactly which field is missing, and the fix is to write the field's own name.

### Notes

- **Evidence, with its corpus named.** The differential was re-run for this release against the 2.57.9 validator (`8632ea9`) over **62 rows** — the 34-row format corpus plus the 28-row acceptance corpus, both exported from `validate-plan.test.mjs` and both materialised as real plan directories at CLOSE. Result: 46 unchanged, **13 `ERROR → CLEAN`**, 2 `ERROR → ERROR` with a strictly better diagnosis, and **exactly 1 `CLEAN → ERROR`** — compatibility note 1 above. Nothing here claims a result outside those 62 rows; the acceptance corpus in particular was written from the spec by a different pass than the one that designed the fix, because the previous release's corpus was authored by the pass it graded and missed the class the reviewer then found.
- **`plans/ANCHORS.md` arrives untracked.** On the first `bootstrap.mjs new` after upgrading, the manifest is a new untracked file and no agent step commits it — run `git add plans/ANCHORS.md` once. Until you do, anchor durability is exactly what it was before this release: safe, but silent.
- **Complexity accounting, measured not estimated.** Across the shipped `src/scripts/*.mjs` (tests excluded), comment and blank lines stripped: **+88 / −31, net +57 logic lines** — `validate-plan.mjs` +64/−23 (net +41), `bootstrap.mjs` +22/−6 (net +16), `schema.mjs` and `check-doc-parity.mjs` one line each, net 0. That is **over** the plan's ≤ +40 budget; the overage is the Verdict rule being written twice, once on indentation and once, after review, on label tightness. One file added (`plans/ANCHORS.md`, data), no new module, no new abstraction, no new gate or check slug.
- `TEST_COUNT` 714 → 739 (+25, live run). `make validate` / `make lint` / `make test` exit 0 at this release commit, and were re-run retrospectively in scratch worktrees at each of the 8 commits that existed when the review pass began; the review-fix commit and this one were checked at the tip.
- MINOR bump: a new resolver tier, a new committed artifact, and a widened published grammar — not a pure bug-fix set.
...
- **`EXPECTED_MIN_KEYS` and `EXPECTED_MIN_PROSE_FILES` were floors in name only** — `check-doc-parity.mjs` guarded 16 live File-Ownership keys with a floor of 10, and `check-agent-wiring.mjs` guarded 22 live prose files with a floor of 15. Both gaps were reproduced as exploitable: 6 ownership rows can vanish from both docs in lockstep, and 5 agent prompts plus 2 rule modules can be deleted, with both gates still printing PASS. Both constants are now set to their exact live counts (17 after the `LESSONS-archive` row below, and 22) and each is pinned by a test asserting equality with the count parsed/discovered from the real repo — so a future legitimate addition turns the build red and forces the same deliberate, review-visible bump `TEST_COUNT` already demands. The floors are deliberately NOT auto-derived: a self-deriving floor ratifies whatever is on disk, which is the vacuity the floor exists to prevent.
- **`checkVerificationVerdict` never flagged a `verification.md` left at bootstrap's all-`PENDING` skeleton**, so a never-completed Verdict reached CLOSE clean. The check is now state-gated exactly as `checkCrossFileConsistency` already is — silent outside REFLECT/CLOSE (a PENDING Verdict at PLAN/EXECUTE is *correct*; bootstrap just wrote it), `WARN` at REFLECT (the verifier may be mid-fill), `ERROR` at CLOSE. Reuses the existing `check: "verdict"` slug; no new check slug.
- **The same function raised a false `ERROR [verdict]` on narrative prose above the bullets.** Its five keyword regexes scanned the whole extracted Verdict section, so a sentence mentioning the criteria out of order made a correctly-ordered bullet list look inverted. The scan is now scoped to bullet lines (`/^\s*[-*]\s+/`, tolerating `-`/`*` markers and leading whitespace); genuinely out-of-order bullets still ERROR.
- **`state-pivot.md` instructed a `findings/{topic}.md` write that no owner was permitted to perform.** PIVOT appends `[CORRECTED iter-N]` annotations, but the File Ownership Model named the Explorer sole owner and PIVOT's dispatch spawns no explorer — an instructed write with no writer, violating the "only the owner writes" invariant at the specification level. The Orchestrator now holds a narrowly-scoped co-write for exactly those annotations, recorded in the owner cell of BOTH ownership tables (byte-identical, owner-cell text is exact-match gated), named in `state-pivot.md`, and spelled out as its own numbered step in `ip-orchestrator.md`'s PIVOT Dispatch.
- **Three File Lifecycle Matrix / Mandatory Re-reads cells disagreed with the sources that actually perform the work**, each resolved toward the side with a real performer. (1) The matrix claimed a PIVOT-time `changelog.md` **write** that no source performs and no owner is named for — the write claim is dropped (`W (append REVERT)` → `R`), while the *read* is real and useful (keep-vs-revert needs the per-edit ledger) and was added to `state-pivot.md`'s read list and `ip-orchestrator.md`'s PIVOT dispatch step 1, putting all three sources in agreement. (2) The matrix required PIVOT `R(600)` reads of `plans/FINDINGS.md` and `plans/DECISIONS.md` that two independent sources omit; both cells become `—`, since PIVOT always transitions to PLAN, which reads `plans/DECISIONS.md` immediately after. (3) The Mandatory Re-reads table omitted the `plans/DECISIONS.md` read the matrix and orchestrator both require — added, annotated `(PLAN only)` to stay consistent with (2).
- **`plans/LESSONS-archive.md` was written by the Archivist but documented nowhere.** Added to SKILL.md's Filesystem Structure tree (conditional), the File Lifecycle Matrix (`—` through PIVOT, `W (overflow archive)` at CLOSE), and both ownership tables as an identical row — taking them from 16 to 17 keys. Its readers cell is `—`, not `Orchestrator`: no protocol step reads the file.
- **`make build-combined` left literal `` `references/root-cause-analysis.md` `` citations in the single-file build.** The doc was added in v2.55.x but never entered either rewrite map, so its citations survived inlining while the other 8 reference files' were rewritten. Two `sed` lines added to the `Makefile` and the two matching `$refMap` entries to `build.ps1`, in lockstep, rewriting both path forms to "the Root Cause Analysis Methods Reference section below". Verified by a real `make build-combined` plus a grep of the built output: zero backticked reference-path citations survive for any of the 9 reference files.
- **`src/scripts/shared.test.mjs` was missing from both repository-structure trees.** The finding claimed only CLAUDE.md was affected; re-checking at PLAN time showed README lists the file in its `node --test` command list but not in its tree either. Both trees fixed in the same commit.
- **`CHANGELOG_SPEC` declared two element shapes nothing constructs or reaches.** `compressed` and `compressed-summary` — residue of the reverted v2.33.0 XML-changelog experiment — were unreachable: `entryFromFields()` builds only `<entry>`, and `checkChangelogFormat` skips compressed lines before the driver runs. Both definitions and their entries in the `changelog` children map are deleted per the repo's own "deleted, not extended" precedent, leaving `{ entry: "*", raw: "*" }`; the doc comment no longer claims to declare the compressor's shapes. The driver's `?`-cardinality and int/range typing behavior keeps coverage via a small synthetic spec defined in the test file, so only the unreachable *declarations* went. Falsification check passed before committing: `validate-plan.mjs` output over a real compressed-changelog fixture was byte-identical before and after.

### Notes

- `make validate`/`make lint`/`make test` all green (707/707 tests; doc-parity PASS at 17 keys, agent-wiring, template-parity, changelog-parity, readme-parity, register-density all PASS), re-run at the end of every one of the 9 steps, not only at the end.
- `TEST_COUNT` 696 → 707 (+11), all from live runs, never hand-computed: +8 verdict regression tests (PLAN/REFLECT/CLOSE PENDING gating, the narrative-above-bullets false-positive reproduction verbatim from the finding, and out-of-order bullets still ERRORing), +1 schema reachable-elements ratchet, +2 floor live-count pin tests. README's badge and per-suite breakdown were updated to the live counts in the same commits.
- `register-baseline.json` unchanged — no touched doc's jargon-marker density rose past its committed ceiling; the peak (CLAUDE.md) actually fell 11.99 → 11.97/1k.
- PATCH bump per this repo's established "bug fixes = patch" convention — all 12 are corrections to already-shipped behavior, no new feature. Zero files added, zero new abstractions, zero new gates or check slugs.
- **Accepted limitation — `build.ps1`'s half of the rewrite-map fix is static-read-verified only.** No PowerShell runtime was available in this environment, so the two new `$refMap` entries were read against the Makefile's verified pair and the file's own single-quote/backtick convention, never executed. The same limitation the 2.57.6 release accepted for `Invoke-SyncSkill`.
- **Accepted limitation — 6 un-backticked `references/x.md` citations remain in the combined build**, all inside the inlined `file-formats.md` plan-file template skeletons. They are template *body* text agents write into plan files, not cross-references, they affect 4 reference files equally, they pre-date this release, and rewriting them would corrupt the byte-parity-gated templates. Deliberately not fixed.
- A falsification signal fired during the floor bump: raising `EXPECTED_MIN_KEYS` to 17 broke TWO `check-doc-parity.test.mjs` fixtures, not the one the plan mapped. The full cascade was enumerated empirically in a scratch copy before any edit (6 files mention either constant; both failures carry the identical floor diagnostic; `check-agent-wiring.test.mjs` has zero cascade at 22) and judged same-class before continuing. `CHANGELOG.md`'s hardcoded `= 10`/`= 15` in the 2.56.0 entry are append-only release history and were deliberately NOT rewritten.
...
- **`runPreStepGate` and `checkCheckpoints` trusted the agent-declared `## Iteration: N` field alone**, unlike `checkIterationLimits`, which already reconciles it against real `EXECUTE → REFLECT` transition-history records. Both now use `max(declared, deriveIterationFromHistory(state))`, closing a path where a forgotten iteration bump could silently bypass the iteration-6 HARD cap or the checkpoint-existence WARN. No new file I/O — both still open only `state.md` and stay well under the `<50ms` `--pre-step` budget. A same-iteration completion-fix pass caught the identical declared-only read surviving in two more places: `checkCrossFileConsistency`'s Convergence-Metrics WARN (now the same `max(declared, derived)` reconciliation) and `bootstrap.mjs`'s `resume`/`status` CLI display (now shows the reconciled value, via a small stripped-only duplicate kept independent of `validate-plan.mjs` for install-layout reasons — see decisions.md).
...
- `make lint`/`make validate`/`make test` all green post-edit (696/696 tests — 8 new regression tests across both completion-fix rounds: pre-step-gate + checkCheckpoints + checkCrossFileConsistency derived-iteration fixtures, `.yml`/`.jsx` new-extension anchor scanning, `.yml` retire-stamping, plus 2 `bootstrap.mjs` display-function fixtures added in round 2 below); `TEST_COUNT` bumped 688 → 696 and README's per-suite breakdown (`bootstrap` 239 → 242, `validate-plan` 117 → 122, aggregate 688 → 696) updated to match the live counts, not hand-computed.
...
- **Completion-fix pass** (same iteration, before CLOSE): the iteration-1 adversarial review (`ip-reviewer`) returned NEEDS_WORK on 5 WARNING-level findings against the initial 693-test release above. Beyond the iteration-trust extension noted above, this pass also: backfilled 4 `changelog.md` lines that still said `uncommitted` for steps that were in fact committed; corrected a stale `decisions.md` D-002 **Anchor-Refs** pointer (an append-only correction entry, not an in-place edit); and documented two accepted, pre-existing architectural risk notes rather than opening new code changes — (1) the exit-2 HARD gate's derived-iteration computation can deliberately over-count (D-009) and its explaining `[state-comment-anomaly]` WARN is unreachable from `--pre-step`, now inherited at 2 more call sites; (2) growing `ANCHOR_SOURCE_EXTS` widens `retire`'s existing whole-file (non-span-scoped) write to 16 more extensions, which is a real, unmodeled risk on non-`.md` files in CONSUMER repos specifically (not this one).
- **Completion-fix pass 2** (same iteration, before CLOSE): a follow-on adversarial review pass re-audited the completion-fix above and returned NEEDS_WORK on 3 more WARNINGs, all about the fix itself: (1) the accepted-risk note for `bootstrap.mjs`'s `resume`/`status` display named only the HARD gate's over-count risk, never the asymmetric opposite-direction risk the reviewer demonstrated live (a stray `<!--` blanking Transition History makes the display under-report toward the stale declared value) — documented in `decisions.md` and cross-referenced from `displayIteration`'s doc comment, no behavioral change; (2) `countExecuteReflectStripped`/`displayIteration` had zero automated test coverage — 2 new `bootstrap.test.mjs` regression tests added (stale-declared-low/derived-high fixture, fresh-plan no-false-positive fixture); (3) `verification.md` was stale (still showed pass-1's original 121/121, 693/693 counts with no row for either completion-fix round) — rewritten to reflect the true current state.
...
- **`code-hygiene.md`'s documented Change Manifest checkbox syntax was essentially unused in real practice** (at most 1 of 27+ sampled plans — a single legacy-format outlier). Real practice converged independently on a per-step format (`- Step N (commit-or-uncommitted): \`file1\`, \`file2\``, backticked filenames required). Documentation now follows practice: the per-step format is canonical in `code-hygiene.md`, and `file-formats.md`'s worked `TEMPLATE:state` example was aligned to match (the byte-gated `SKELETON:state` region was untouched — it already carried no checkbox syntax).
