...
- **The skip-line convention, `- HYGIENE SKIP (iter N): <reason>`, lives inside the already-existing Transition History bullet list, not a new `state.md` section.** It is deliberately arrow-free, so `checkStateTransitions`'s `FROM → TO` regex never reads it as a state transition and no `REFLECT → REFLECT` entry had to be added to `VALID_TRANSITIONS`. A dedicated regression test proves the arrow-free line never trips `[transition]`, rather than trusting the regex reading alone. This plan (`plan-2026-09-11T141919-e5db2894`) considered and rejected a new `state.md` section for the same reason it considered and rejected a new `--pre-close`-style CLI mode with its own reserved exit code (D-001, D-003): both are heavier and riskier than reusing machinery this repository has already proven — the existing `currentState === "CLOSE" ? "ERROR" : "WARN"` severity escalation, already load-bearing at `checkVerdictBullets`, needed no new exit code and no second `--help` block.
...
- This closes the exact gap the goal named: before this release, whether the hygiene sweep ran or was legitimately skipped before CLOSE was pure orchestrator judgment, with nothing mechanical that verified either. It now escalates WARN at REFLECT to ERROR at CLOSE, mirroring `[atlas-cap]`'s precedent rather than inventing a new mechanism.
- Test count: 963 (was 949 — net +14, live run; `TEST_COUNT` synced mechanically to the live `node --test` pass count).
