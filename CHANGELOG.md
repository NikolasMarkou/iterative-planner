...
- **Viability-flipping constraint rule** — before generating candidates, the agent must scan classified constraints for any whose reclassification (hard ↔ soft ↔ ghost) would flip a candidate's viability. If classification is uncertain → ASK the user a focused question; don't guess. If discovered mid-generation, the agent stops, re-classifies (with `[CORRECTED iter-N]` if applicable), and re-runs the gate. State remains EXPLORE — the gate is an EXPLORE sub-step, not a separate state.
