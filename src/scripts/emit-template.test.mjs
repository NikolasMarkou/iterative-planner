...
// --- Buffer-input fidelity (D-009) --------------------------------------------
// resolveTemplate's byte source may be a pre-read Buffer as well as a URL/path, so the checker reads
// file-formats.md ONCE and resolves all 17 served bodies from that Buffer (and tests can resolve from
// an in-memory mutated doc). Passing a Buffer must be BYTE-IDENTICAL to reading the file — the
// Buffer.indexOf slicing is untouched. This is the guard on that behavior-preserving change.

test("resolveTemplate(slug, Buffer.from(text)) byte-equals resolveTemplate(slug) — the checker's read-once path", () => {
  const docBuf = Buffer.from(readFileSync(fileFormatsPath, "utf8"));
  for (const slug of VALID_TEMPLATES) {
    const fromFile = resolveTemplate(slug);
    const fromBuffer = resolveTemplate(slug, docBuf);
    assert.equal(fromFile.ok, true, `${slug} did not resolve from the file`);
    assert.equal(fromBuffer.ok, true, `${slug} did not resolve from the Buffer`);
    assert.ok(fromBuffer.body.equals(fromFile.body), `${slug}: Buffer-input slice != file slice`);
  }
});
