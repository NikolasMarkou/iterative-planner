- Read `decisions.md`, `findings.md`, relevant `findings/*`, `plan.md`, `verification.md`, `plans/LESSONS.md`, `plans/SYSTEM.md`.
...
- If earlier findings proved wrong or incomplete → the orchestrator appends the corrections itself to `findings.md` + `findings/*` (no explorer is spawned at PIVOT; the File Ownership Model grants the orchestrator this narrow co-write on `findings/{topic}.md`). Mark corrections: `[CORRECTED iter-N]` + what changed and why. Append, don't delete original text.
