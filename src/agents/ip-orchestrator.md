...
Only this agent declares `memory: project` — deliberate, not an oversight: the six worker agents are spawned per-task with full explicit context (plan-dir file reads) and exit after one deliverable, so they have no cross-invocation state to persist; the orchestrator alone spans the plan lifecycle.
...
2. Verify — confirm all of (unordered; the archivist's own Steps 1-6 order is authoritative): summary.md written; decision anchors audited; LESSONS.md + SYSTEM.md updated with their post-rewrite validator gates run clean (ip-archivist Steps 3-4); close ran; consolidated files compressed if >500 lines (ip-archivist Step 6)
