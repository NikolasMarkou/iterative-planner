...
7. Reuse-before-write (DRY): before adding a new function, constant, parser, or rule, `grep -rn` for an existing one. If the behavior or fact already exists, import/reference it — do not re-implement. If a value or rule must live in 2+ places, that is a duplication smell: centralize it, or record in decisions.md why it cannot be (a hand-maintained "kept in lockstep" invariant is a defect, not a pattern). Conversely, when you CREATE a new function/module meant for reuse (2+ callers), document a short interface contract (parameters, return shape, failure mode) at its definition site — `references/code-hygiene.md` § Interface Contracts for Shared Assets; undocumented shared code is itself a hygiene leftover.
...
- Commit after success: `[plan-YYYY-MM-DD-HASH/iter-N/step-M] description`. Derive the tag id from the plan-dir name by dropping the `THHMMSS` segment (`plan-2026-07-14T051317-317362c4` → `[plan-2026-07-14-317362c4/iter-3/step-2] …`). Legacy `plan_YYYY-MM-DD_XXXXXXXX` dirs derive identically (normalize `_`→`-`); full rule in `SKILL.md` § Git Integration.
  - **The changelog `step` field carries no plan-id prefix** — it always names a numbered step, `iter-N/step-M`, or `iter-N/step-M.K` when a completion fix repairs step M. Do not prefix it with the tag id. It is sourced from `state.md`, never parsed from the commit subject.
