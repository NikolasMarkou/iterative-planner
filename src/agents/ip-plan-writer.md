...
- **Software-engineering plans (any language, incl. Python)**: also read `references/python-software.md` for software-design models and Python architecture patterns before drafting steps. Skip for non-software plans. Record a Domain-Caveat Consult Note in `decisions.md` per `references/file-formats.md`'s convention.
