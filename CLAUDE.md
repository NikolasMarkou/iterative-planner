...
└── src/
    ...
    │   ├── ip-boyscout.md            # Read-only hygiene sweep (REFLECT phase, once the work looks close-ready)
    ...
    │   ├── check-doc-parity.test.mjs # Test suite (node:test); also holds the seven Makefile <-> build.ps1 lockstep tests (a home of convenience, not a claim that build-channel parity is doc parity)
    ...
    │   ├── check-register.mjs        # Register ratchet gate: measures the jargon markers (bracket-tags + coded refs + 3+-segment compounds) of shipped docs (CLAUDE.md, README, SKILL.md, agents, references) as both a raw count and a density per 1k words, against the committed per-file entry in register-baseline.json; ERRORs [register-drift] only when BOTH rise past the committed pair (so deleting plain prose, which raises density on its own, can never fail), [register-floor] on missing/empty/too-few docs or a malformed entry; `--regenerate` rewrites the baseline from the live docs, lowering or holding any ceiling freely but refusing (`[register-raise]`, exit 1) to raise one unless that file is named with `--raise <relpath>` (repeatable) (used by make/build.ps1 validate+lint+test; Node.js 18+)
    │   ├── check-register.test.mjs   # Test suite (node:test)
    │   ├── register-baseline.json    # Per-file `{ceiling, markers}` pairs consumed by check-register.mjs — the exact live measurement, regenerated with `--regenerate` (gate-data, not a script)
    │   ├── scar-scan.mjs             # Hygiene scanner, run by ip-boyscout at REFLECT — a REPORT, not a build gate (in both channels' lint+test lists, deliberately NOT in validate). Five sweep categories: orphaned decision anchors (delegated to validate-plan.mjs, never re-declaring the anchor grammar), stale `uncommitted` changelog commit fields, orphan plan-directory artifacts, the Forbidden Leftovers code sweep from code-hygiene.md, Complexity Budget reconciliation; every finding partitioned INHERITED vs INTRODUCED. Exit 0 whenever the scan completed, REGARDLESS of what it found; exit 1 ONLY on [scan-unavailable] (upstream validator absent, crashed, timed out, or printed no parseable proof-of-run — stdout stays empty on this path, even under `--json`) or [scan-floor] (fewer categories reported a status than EXPECTED_MIN_CATEGORIES). Cost: adds NO full-corpus walk over `plans/` — categories B, C and E read ONE plan directory (O(1) in plan-dir count), A delegates to validate-plan.mjs's already-documented O(referenced-plans), D is O(source files in scan scope) (Node.js 18+)
    │   ├── scar-scan.test.mjs        # Test suite (node:test)
...
- [ ] Register density of the shipped re-read docs (CLAUDE.md, README.md, src/SKILL.md, src/agents/*.md, src/references/*.md) stays at or below the committed per-file ceilings in `src/scripts/register-baseline.json` (enforced by `node src/scripts/check-register.mjs`, run via `make validate`): FAIL `[register-drift]` when a file's density rises past its ceiling AND its raw marker count rises past the committed count — **both, never one**, FAIL `[register-floor]` (anti-vacuity) when a baseline-listed doc is missing, near-empty, malformed, or fewer files are scanned than baseline keys. The countermeasure to the register self-overfit loop; C1 setpoint lives in SKILL.md § Register Discipline, C3 actuator in ip-archivist Steps 4-5. **Every ceiling is the file's exact measured density, not a rounded-up allowance.** The first baseline was generated with a uniform allowance added to each measurement, which made the gate a loose ceiling with 17% to 100% headroom rather than a ratchet: density could rise a long way with the build staying green. Ceilings were re-measured at HEAD in v2.61.0 and every one of the 19 fell. So adding one marker to a doc turns the build red, and that is the intent — the reply is to write the sentence plainly, or, if the marker is genuinely earned, to raise that one ceiling in the same commit where a reviewer can see it. `EXPECTED_MIN_FILES` is pinned to the live scanned doc count by a `pin:` test in `check-register.test.mjs`, the same idiom the other three floor-carrying gates use, and the baseline key count is pinned to that number too — so a new agent or reference doc with no entry, or an entry left behind by a deleted doc, fails rather than passing quietly.

  **Why the rule needs two numbers, not one.** Density is markers divided by words, so removing plain prose raises it while adding no jargon at all: the density-only form of this gate failed a doc for having twelve ordinary sentences deleted, and its own failure message said density could fall freely. That punished exactly the plainer writing SKILL.md § Register Discipline asks for. Requiring the raw count to rise as well makes a pure deletion structurally unable to fail, because deleting text cannot raise a count. Nothing else changes: adding a marker still turns the build red, and adding plain prose still passes.

  **The baseline is a record, not an allowance, and a test says so.** A second `pin:` test asserts that every committed entry EQUALS its file's live measurement — both numbers — so an entry quietly set above reality (the very defect the v2.61.0 re-measure removed) fails the suite instead of passing every gate. The consequence is that any edit which moves a doc's measurement must move the baseline in the same commit: run `node src/scripts/check-register.mjs --regenerate`, read the diff, commit it. That diff is the review, which is why regenerating is a visible act and not a quiet one. A companion test regenerates into a throwaway copy of the docs and asserts the bytes match the committed file, so the reproduce-it instruction in this bullet is executable rather than merely stated, and `--regenerate` refuses to write at all from a scan too small to trust.

  **Regenerating may lower a ceiling, but it may not raise one behind your back.** Requiring both numbers to rise leaves one shape the gate cannot see: swap jargon in and out at an equal count, deleting plain prose as you go, and the count holds while density climbs. That shape passes the gate, fails the equality pin test, and the pin test's one repair is a regenerate — so an unguarded regenerate was itself the way to launder new jargon in. It has happened here: a regenerate in this plan quietly lifted one reference doc from 3.33 to 3.8. So `--regenerate` now lowers or holds a ceiling with no flag at all, and refuses to raise one unless you say which file: `node src/scripts/check-register.mjs --regenerate --raise CLAUDE.md` (repeat the flag per file). A refusal exits 1, leaves the baseline untouched, lists every ceiling that would have risen with its old and new value, and prints the command to re-run. Raising a ceiling is meant to feel like bumping `TEST_COUNT`: a deliberate line in a diff that someone can ask about. The refusal fires on the density only, because density is the number the gate compares first; a marker count that rises while density falls is the dilution case the two-number rule already accepts.
...
- [ ] `plans/ANCHORS.md` should be committed and stay committed — **and in this repository it is not, today.** Bootstrap writes the plans glob plus a negation line for it into `.gitignore`, so the file is addable and `git ls-files plans/` should print exactly that one path and nothing else under it. It prints nothing. The negation permits the add; nothing ever performed it. The file is the durable tier of anchor resolution in `validate-plan.mjs`: every other source a `# DECISION` anchor can resolve against lives inside the ignored plans directory, so it is gone in a fresh clone — which is the mechanical cause of the 75 orphaned anchors the scar scanner reports as inherited backlog. Repairing that is a real change to what this repository ships and belongs in a plan of its own, not in a footnote; until then this bullet reads as a target, not a description. It is deliberately **not** a `PLAN_TEMPLATES` entry — a 13th key would drag in a new skeleton region, a bumped slug floor, an 18th served slug with its own test, and every "12 templates / 17 served" sentence in this file, all to publish a four-line header; a bootstrap test pins those header bytes instead. Format and rules: `src/references/file-formats.md` § plans/ANCHORS.md
...
- [ ] **The two build channels fail on the same conditions.** `build.ps1` must not wrap a mandatory script or gate in a `Test-Path` skip: the Makefile greps those files directly and dies when one is gone, so a skip wrapper made a deleted gate pass validation silently on Windows only. A missing script is an error, never a skip. The file also declares `#Requires -Version 7` on its first line and names `-Encoding utf8` on every `Get-Content` and `Set-Content`; under Windows PowerShell 5.1 both default to the ANSI codepage, and SKILL.md is dense with em dashes, arrows and section signs that a round-trip would corrupt. The `#Requires` floor is a **compatibility break for the Windows channel** and is disclosed to users in README's build section and the 2.61.0 CHANGELOG entry, not only here. Seven lockstep tests bind the channels — same validate gate set, no skip-wrapped gate, same lint list, same test list, same build-combined rewrite pairs, the same trailing combined-file Note (compared DECODED, since Make escapes a backtick as `\``and PowerShell as ` `` `), and the version guard plus encoded round-trips. They live in `check-doc-parity.test.mjs` today, which is a home of convenience rather than a statement that build-channel parity is doc parity; move them if a better home appears. **No PowerShell runs in the usual development environment here, so every claim about that channel is verified by reading it, not by running it** — say so when you report on it
- [ ] **Both channels' `build-combined` rewrite the per-state and per-template pointers**, not just the reference citations. Neither router runs in a paste context, and the five module bodies are already inlined as `## State Module: <state>` sections, so a surviving `node <skill-path>/scripts/emit-state.mjs` line in the combined file tells a reader to run something they cannot run. The two rewrite maps are one fact in two places and a lockstep test compares them pair by pair. Acceptance: the combined output contains no `node <skill-path>/scripts/emit-` line. **That is the whole claim, and it is deliberately narrower than "ships no pointer to a script it does not ship"** — roughly 22 `bootstrap.mjs` / `blast-radius.mjs` / `validate-plan.mjs` pointers survive in the inlined bodies on purpose. They are protocol prose naming real commands of the full package, and rewriting each one would strip the reader's only record that the command exists. Instead the combined file's trailing Note says once, plainly, that NO script pointer in it is runnable as written. The Note sits at the END because SKILL.md's YAML frontmatter must remain the file's first bytes — a preamble cannot go above it
- [ ] **A `for` loop in the Makefile has to fail the build from ANY iteration, not just the last one.** Six loops in the `validate` target used the shape `for x in …; do <test> || ( echo "ERROR…" && exit 1 ); done`. The `exit 1` runs inside the `( )` subshell, so it ends that subshell and nothing else, and a shell `for` loop exits with the status of its LAST iteration — whichever entry happened to be listed last was the only one enforced. The other five printed their ERROR line and the target still exited 0, and `Validation passed!` printed underneath. The six were the cross-reference check, the bootstrap file-list check, the bootstrap directory check, the agent-frontmatter check, the Mermaid-edge check, and the validator transition-coverage check. `build.ps1` never had the defect: it accumulates into `$errors` and exits 1 when the count is non-zero, which is the mirror image of the `Test-Path` skip bug above with the two channels' roles swapped. All eight call sites now use a brace group (`{ echo …; exit 1; }`) so a failure anywhere in the loop fails the target; the single-line `|| ( … && exit 1)` uses OUTSIDE any loop are correct and were left alone. **Probe any repair here with a NON-LAST entry** — a probe against the last entry passes either way and proves nothing, which is how this survived unnoticed
- [ ] **The state-machine consistency check asserts real diagram edges.** Both channels match the indented `FROM --> TO` line in SKILL.md's Mermaid diagram, for all ten transitions including `REFLECT --> EXECUTE`. The predecessor matched `FROM.*TO` anywhere in the file, which meant `PLAN.*PLAN` could not fail on any document that mentioned PLAN twice on one line. Removing an edge from the diagram must turn the build red; if it does not, the check is decorative. It WAS decorative in the Makefile until v2.63.0, and not because this pattern was wrong: the loop around it could not fail from a non-last iteration, so nine of the ten edges could be deleted with the build staying green. The claim is only as good as the loop that carries it — see the bullet above
- [ ] **The validator fires only on non-conforming input**, and there is no Presentation Contract check. A `presentation-contract-unlogged` warning lived in `validate-plan.mjs` from v2.17.0 and demanded a contract name appear in a plan file, though no protocol file ever told an agent to write one there — so a correctly run plan was warned at every gated transition. It was deleted rather than made satisfiable, because an agent that logs a contract name proves only that it logged the name. Two sibling repairs go with it: `findings/` holds more than one artifact schema, so `review-iter-N.md` and `review-iter-N-passM.md` are linted against the reviewer's own Concerns / Blind Spots / Verdict sections instead of the explorer's — the discriminator switches the required list, it does not exempt those files, and a review file missing its Verdict must still warn. And the verification-evidence check skips a row whose Criterion cell is still placeholder text, reusing the validator's existing placeholder patterns rather than growing a second exemption list. Do not widen that skip to the Evidence cell: an empty Evidence cell beside a real criterion is exactly what the check exists to report
...
- [ ] **The scar scanner reports; it never gates, and it refuses to produce a false all-clear.** Findings never fail a build — the only non-zero exits are the two ways the scan itself becomes untrustworthy: `[scan-unavailable]` when the upstream `validate-plan.mjs` is absent, crashed, timed out, or printed nothing the parser accepts as proof it really ran, and `[scan-floor]` when fewer categories reported a status than `EXPECTED_MIN_CATEGORIES`, which a `pin:` test holds EQUAL to `CATEGORIES.length` (5 today), the same exact-floor idiom as the other three floors. On the untrustworthy path stdout stays **empty**, even under `--json`: a consumer reading an empty introduced list could not tell it from a clean repository. There is deliberately NO floor on the number of findings — the correct end state is zero, and a constant pinned at today's count would forbid ever reaching it. Two holes are disclosed rather than closed: a category that runs but silently matches nothing because its own pattern rotted (only the per-category fixtures cover that, which is weaker than a floor), and `console.log` / `print()`, which the Forbidden Leftovers list names but the sweep refuses to flag, because every gate in this repo is a CLI whose entire output channel is `console.log` and a rule that fires on hundreds of correct lines is a rule that gets ignored
- [ ] **The partition attributes an anchor by its identity, never by which files a plan touched.** For orphaned anchors an item is this plan's ONLY when its plan-id resolves AND equals the active plan-id; for every other category the changelog-membership rule stands. The first implementation applied membership alone, and on its first real run it called three anchors that exist verbatim at the plan's own base commit newly introduced, purely because a later step had added lines above them. That is the noise failure the tool exists to prevent, inverted: the more files a run edits, the more pre-existing residue it appears to have caused. A genuinely new orphan is still caught, since a new orphan carries the active plan-id. Two narrowings pay for it and are written in the scanner's own header — an anchor with no resolvable plan-id always reads inherited, and the unreferenced-checkpoint rule does not fire on the active plan's own in-flight checkpoints
- [ ] **`findings/` holds THREE artifact schemas.** `HYGIENE_FILE_RE` in `checkFindingsTopicSections` matches `hygiene-iter-N.md` and `hygiene-iter-N-passM.md` and switches the required sections to `Inherited` / `Introduced` / `Verdict`. Like the reviewer branch it SWITCHES the required list and never exempts: a hygiene file missing `## Verdict` must still WARN, because REFLECT routing reads that section to decide whether any remediation happens. Three enumeration floors moved with the agent and the ownership row that caused them: `EXPECTED_MIN_PROSE_FILES` 22 → 23 in `check-agent-wiring.mjs`, `EXPECTED_MIN_FILES` 19 → 20 in `check-register.mjs`, `EXPECTED_MIN_KEYS` 18 → 19 in `check-doc-parity.mjs`. Each is pinned EQUAL to its live count by a `pin:` test, so a new agent file, reference doc, or File Ownership row that lands without moving its floor turns the suite red rather than passing quietly
