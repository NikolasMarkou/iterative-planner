...
└── src/
    ...
    │   ├── check-template-parity.mjs # Byte-parity gate: bootstrap.mjs's 12 `PLAN_TEMPLATES` vs the 12 `<!-- SKELETON:<slug> -->` regions in references/file-formats.md (parity, both-direction completeness, encodability, typing, line-endings, duplicate-region, coverage floor, and byte-claim — no claim of bootstrap's bytes before `<!-- TEMPLATE:END -->`; used by make/build.ps1 validate+lint+test; Node.js 18+)
...
- [ ] **The plan-file templates live in TWO places on purpose, the two are mechanically enforced equal, and bootstrap's bytes are CLAIMED in exactly one of them** (v2.39.0). `file-formats.md` has two halves, and confusing them is what made v2.38.0's gate guard the wrong copy. **Worked-example half** (everything *before* `<!-- TEMPLATE:END -->`): the 17 `<!-- TEMPLATE:<slug> -->` regions that `emit-template.mjs` slices at runtime and **serves to agents** — populated illustrations of a filled-in plan file (fictional dates, fake commits, a narrative), *not* what bootstrap writes. **Skeleton half** (the 12 `<!-- SKELETON:<slug> -->` regions *after* that marker): the bytes `bootstrap.mjs new` actually writes. Bootstrap renders new plan files from its OWN exported `PLAN_TEMPLATES` map (raw strings + `{{TOKEN}}` placeholders, via `renderTemplate()`) and performs **zero** runtime reads of `file-formats.md` — deliberate and load-bearing, since bootstrap is the one script whose failure mode is "no plan can ever be created again" (the runtime-read alternative was rejected on risk, not feasibility). So the duplication is guarded, not removed: `src/scripts/check-template-parity.mjs` (`make validate`, plus `lint`/`test`) byte-compares the 12 `PLAN_TEMPLATES` entries against the 12 skeleton regions in **both directions**, and cannot pass vacuously (coverage floor `EXPECTED_SLUGS = 12`; duplicate `<!-- SKELETON:x -->` markers rejected first-wins; a non-string template reported, not thrown on; a CRLF doc gets one hint instead of 12 unexplained failures). **A template edit is two edits — `PLAN_TEMPLATES` and its `<!-- SKELETON:<slug> -->` region — and forgetting the second turns the build red naming the slug and the divergent line.**

  **The two halves are NOT byte-compared to each other, and must not be** — for `system` they differ *deliberately*: the doc's `## plans/SYSTEM.md` section is the **populated-form schema** `ip-archivist` fills at CLOSE (which is why `ip-archivist.md:43` points there), while bootstrap writes an **unpopulated** skeleton (`(none yet)` sentinel, UNPOPULATED banner, every hint bullet italicized). What the worked-example half must never do is **claim** bootstrap's bytes. v2.38.0 gated the skeletons while the `changelog` worked example still restated bootstrap's header verbatim — an un-gated second copy, free to drift, and `emit-template` served *that* one to agents. It is **deleted**, replaced by a pointer to `<!-- SKELETON:changelog -->`. Re-introducing a byte-claim before `<!-- TEMPLATE:END -->` is now caught **twice**: rule (h) `[byte-claim]` in the gate (`make validate`) and a suite assertion pinning the deleted block's literal (`make test`). Rule (h) is a 4-phrase **set**, not an allowlist — **if it ever needs a 5th phrase or a per-slug exception, DELETE the rule; do not grow it** (a phrase list that needs exceptions is the allowlist design that rotted `check-doc-parity`)
