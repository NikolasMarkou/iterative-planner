...
- [ ] **The changelog's `step` field always names a numbered step — `iter-N/step-M` — even in a completion-fix loop.** A completion fix runs REFLECT → EXECUTE without raising the iteration, so it is tempting to write `iter-1/completion-fix` there. That value fails the `step` regex in `schema.mjs`, and every line carrying it turns into a `WARN [changelog-malformed]` from `validate-plan.mjs`. It has happened in two plans in this repo (`plan-2026-07-31-de0ded98` and `plan-2026-08-04-3a5913b0`, 7 warned lines). Use the plan step's own number: if the fix is plan.md step 11, the field is `iter-1/step-11`. The commit subject may say whatever is clearest; only this field is checked.
