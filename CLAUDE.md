...
└── src/
    ...
    │   ├── check-template-parity.mjs # Byte-parity gate: bootstrap.mjs's 12 `PLAN_TEMPLATES` vs the 12 `<!-- SKELETON:<slug> -->` regions in references/file-formats.md (parity, both-direction completeness, encodability, typing, line-endings, duplicate-region, coverage floor; plus byte-claim — a 4-phrase PROSE HEURISTIC over the pre-`<!-- TEMPLATE:END -->` half which is evadable by synonym and does NOT enforce that the worked examples avoid bootstrap's bytes; used by make/build.ps1 validate+lint+test; Node.js 18+)
...
- [ ] **The plan-file templates live in TWO places on purpose, and only ONE of the two halves is mechanically compared to bootstrap.** `file-formats.md` has two halves, and confusing them is what made v2.38.0's gate guard the wrong copy. **Worked-example half** (everything *before* `<!-- TEMPLATE:END -->`): the 17 `<!-- TEMPLATE:<slug> -->` regions that `emit-template.mjs` slices at runtime and **serves to agents**. **Skeleton half** (the 12 `<!-- SKELETON:<slug> -->` regions *after* that marker): the bytes `bootstrap.mjs new` actually writes. Bootstrap renders new plan files from its OWN exported `PLAN_TEMPLATES` map (raw strings + `{{TOKEN}}` placeholders, via `renderTemplate()`) and performs **zero** runtime reads of `file-formats.md` — deliberate and load-bearing, since bootstrap is the one script whose failure mode is "no plan can ever be created again" (the runtime-read alternative was rejected on risk, not feasibility). So the skeleton half's duplication is guarded, not removed: `src/scripts/check-template-parity.mjs` (`make validate`, plus `lint`/`test`) byte-compares the 12 `PLAN_TEMPLATES` entries against the 12 skeleton regions in **both directions**, and cannot pass vacuously (coverage floor `EXPECTED_SLUGS = 12`; duplicate `<!-- SKELETON:x -->` markers rejected first-wins; a non-string template reported, not thrown on; a CRLF doc gets one hint instead of 12 unexplained failures). **A template edit is two edits — `PLAN_TEMPLATES` and its `<!-- SKELETON:<slug> -->` region — and forgetting the second turns the build red naming the slug and the divergent line.**
  ...
  **KNOWN LIVE HOLE (shipped in v2.39.0 — do not trust this bullet's predecessors, which claimed it closed).** The worked-example half is compared to **nothing**, and four of its examples restate bootstrap's header bytes **verbatim** right now: `index` (4/4 non-blank template lines), `lessons` (3/3), `findings-consolidated` (2/2), `decisions-consolidated` (2/2). If bootstrap changes one of those lines, the doc goes stale, the build stays **green**, and `emit-template --name lessons` serves agents a block bootstrap does not write. The two mechanisms that were claimed to prevent this do not: **(1)** rule (h) `[byte-claim]` is a 4-phrase prose **set** (`written by bootstrap` / `must match exactly` / `update in lockstep` / `written by the executor`) and is **synonym-evadable** — `The freshly created file emitted by bootstrap contains exactly:` sails straight through it; **(2)** the suite assertion pinning the deleted `changelog` byte-claim (`check-template-parity.test.mjs:318`) is **region-scoped** to `<!-- TEMPLATE:changelog -->`, so bytes planted in any other region are invisible to it. Rule (h) is a heuristic that classifies prose, not a comparison of bytes — **if it ever needs a 5th phrase or a per-slug exception, DELETE the rule; do not grow it** (a phrase list that needs exceptions is the allowlist design that rotted `check-doc-parity`)
