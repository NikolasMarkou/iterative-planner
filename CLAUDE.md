...
└── src/
    ...
    │   ├── check-template-parity.mjs # Byte-parity gate: bootstrap.mjs's 12 `PLAN_TEMPLATES` vs the 12 `<!-- SKELETON:<slug> -->` regions in references/file-formats.md (parity + both-direction completeness + encodability; used by make/build.ps1 validate+lint+test; Node.js 18+)
...
- [ ] **The plan-file templates live in TWO places on purpose, and the two are mechanically enforced equal — editing one without the other FAILS `make validate`** (v2.38.0; this bullet's predecessor claimed no such gate existed, which is no longer true). The two copies are wired differently: `emit-template.mjs` *slices* `references/file-formats.md` at runtime for its 17 slugs and stays byte-faithful to it, while `bootstrap.mjs` writes new plan files from its OWN templates — the exported `PLAN_TEMPLATES` map (raw strings + `{{TOKEN}}` placeholders) rendered by `renderTemplate()` — and performs **zero** runtime reads of `file-formats.md`. That zero-file-dependency property is deliberate and load-bearing: bootstrap is the one script whose failure mode is "no plan can ever be created again," so it is never coupled to a doc file (the runtime-read alternative was rejected on risk, not feasibility — a delivery path shipping `scripts/` without `references/` would break every new plan). The duplication is guarded instead: `src/scripts/check-template-parity.mjs` (run by `make validate`, plus `lint`/`test`) compares the 12 `PLAN_TEMPLATES` entries byte-for-byte against the 12 `<!-- SKELETON:<slug> -->` regions at the end of `file-formats.md`, in **both directions** (a template with no region, or a region with no template, is a FAIL), and forbids a template containing a fence or the literal `<!-- TEMPLATE:`. **So a template edit is two edits — `PLAN_TEMPLATES` and the matching `<!-- SKELETON:<slug> -->` region — and forgetting the second turns the build red naming the slug and the divergent line.** Note the worked examples ABOVE the skeleton regions in `file-formats.md` are a *different artifact*: a populated illustration of a filled-in plan file, not what bootstrap writes, and they are NOT byte-compared to anything
